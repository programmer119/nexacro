...
call :banner "1/9  Prerequisite check / portable GitHub CLI bootstrap"
where git >nul 2>&1
if errorlevel 1 (
...
rem GitHub CLI: use an existing installation if available.
where gh >nul 2>&1
if errorlevel 1 (
...
rem No WinGet is required. If gh is still missing, download the official portable
rem Windows x64 ZIP from the latest GitHub CLI release into .tools\gh.
where gh >nul 2>&1
if errorlevel 1 (
  echo [INFO] GitHub CLI ^(gh^) is not installed. Downloading the official portable build...
  where powershell >nul 2>&1
  if errorlevel 1 (
    echo [ERROR] PowerShell is not available, so GitHub CLI cannot be downloaded automatically.
  ...
  powershell -NoProfile -ExecutionPolicy Bypass -Command "$ErrorActionPreference='Stop'; [Net.ServicePointManager]::SecurityProtocol=[Net.SecurityProtocolType]::Tls12; $headers=@{'User-Agent'='nexacro-apply'}; $r=Invoke-RestMethod -Headers $headers -Uri 'https://api.github.com/repos/cli/cli/releases/latest'; $a=$r.assets ^| Where-Object { $_.name -match '^gh_.*_windows_amd64\.zip$' } ^| Select-Object -First 1; if(-not $a){ throw 'Windows amd64 ZIP asset was not found in the latest GitHub CLI release.' }; $zip=Join-Path '%ROOT%.tools' 'gh.zip'; $dst=Join-Path '%ROOT%.tools' 'gh'; if(Test-Path $dst){Remove-Item -Recurse -Force $dst}; Invoke-WebRequest -UseBasicParsing -Headers $headers -Uri $a.browser_download_url -OutFile $zip; Expand-Archive -Path $zip -DestinationPath $dst -Force; Remove-Item -Force $zip"
  if errorlevel 1 (
  ...
  set "LOCAL_GH_DIR="
  for /r "%ROOT%.tools\gh" %%G in (gh.exe) do if not defined LOCAL_GH_DIR set "LOCAL_GH_DIR=%%~dpG"
